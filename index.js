...
function createMenuItem(name, price, category) {
  /*Your code here*/
  let foodObject = { name: name, price: price, category: category };
...
createMenuItem("tacos", 8, "breakfast");
createMenuItem("chips", 7, "lunch");
...
const burger = {
  ...
  discount(value) {
    if (value.toLowerCase() == "teacher" || value.toLowerCase() == "student") {
...
function addReview(arr, name, rating, feedback) {
  /*Your Code Here */
  let newObj = { name: name, rating: rating, feedback: feedback };
  arr.push(newObj);
  return arr;
}
...
function getReviewByIndex(arr, index) {
  ...
  let review = `${name} gave the restaurant a ${rating} star review, and their feedback was: ${feedback}`;
  return review;
}
...
function getLastReview(arr) {
  ...
  let lastReview = `${name} gave the restaurant a ${rating} star review, and their feedback was: ${feedback}`;
  return lastReview;
}
